...
import { Play, CheckCircle2, Code2, Store, Lightbulb, X, ShoppingBag, Trophy, BookOpen, Star, Info, ChevronRight, ChevronLeft } from 'lucide-react';
...
// 진열대 상품 컴포넌트
const StoreShelfItem = ({ name, price, isSoldOut }: { name: string, price: number | null, isSoldOut: boolean }) => (
  <div className="product-token">
    <ShoppingBag size={40} color={!isSoldOut ? "#2ec4b6" : "#cbd5e0"} />
    <span style={{ fontSize: '0.8rem', fontWeight: 800, marginTop: '8px', textAlign: 'center' }}>{name}</span>
    {price !== null && <span style={{ fontSize: '0.75rem', color: '#64748b' }}>{price.toLocaleString()}원</span>}
    <AnimatePresence>
      {isSoldOut && (
        <motion.div
  ...
  return (
    <motion.div style={{ display: "flex", flexWrap: "wrap", justifyContent: "center" }} variants={container} initial="hidden" animate="visible">
      {letters.map((letter, index) => (
        <motion.span variants={child} key={index} style={{ fontSize: '1.5rem', fontWeight: 700, color: '#1e293b', marginRight: letter === ' ' ? '8px' : '0' }}>
          {letter}
        </motion.span>
      ))}
    </motion.div>
...
const stage1Data: StageData[] = [
  ...
  { id: 7, title: "연습 3: 판매 여부 설정", type: 'coding', description: "상태를 나타낼 때는 True 또는 False만 가지는 불린형(bool)을 사용합니다.", missionGuide: "is_sale 변수에 True 또는 False를 저장하세요.", initialCode: "product_name = \"딸기우유\"\nproduct_price = 1500\nis_sale = ", hint: "True 또는 False를 입력하세요. (첫 글자는 대문자!)", validation: (vars) => {
    if (vars.is_sale === true) return { success: true, message: "상품 등록 성공! 진열대에 표시됩니다." };
    if (vars.is_sale === false) return { success: true, message: "품절 상태로 설정되었습니다!" };
    if (typeof vars.is_sale !== 'boolean') return { success: false, message: "파이썬에서는 첫 글자를 대문자로 쓴 True 또는 False를 사용해야 합니다." };
...
export default function App() {
    ...
    try {
      runPython(code, (vars) => {
        ...
        if (err.includes("NameError: name 'true' is not defined") || err.includes("NameError: name 'false' is not defined")) {
          friendlyMsg = "파이썬에서는 첫 글자를 대문자로 쓴 True 또는 False를 사용해야 합니다.";
  ...
  // 프롤로그 화면 렌더링
  if (!isStarted) {
    const pData = prologueData[prologueStep];
    return (
      <div className="app-shell" style={{ justifyContent: 'center', alignItems: 'center', backgroundColor: '#f1f5f9' }}>
        <AnimatePresence mode="wait">
          ...
          >
            <img 
              src={pData.image} 
              alt="prologue" 
              style={{ maxHeight: '400px', width: 'auto', marginBottom: '2rem', borderRadius: '24px' }} 
            />
            <div style={{ minHeight: '100px', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '3rem' }}>
              <TypingText key={`text-${prologueStep}`} text={pData.text} />
            </div>
            
            <div style={{ display: 'flex', justifyContent: 'center', gap: '20px' }}>
              <button 
                className="btn-base btn-prev" 
                onClick={() => setPrologueStep(s => s - 1)} 
                disabled={prologueStep === 0}
                style={{ visibility: prologueStep > 0 ? 'visible' : 'hidden', display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer' }}
              >
                <ChevronLeft size={20}/> 이전
              </button>
              
              {prologueStep < prologueData.length - 1 ? (
                <button 
                ...
                <button 
                  ...
                  style={{ padding: '12px 30px', backgroundColor: '#3b82f6', cursor: 'pointer' }}
                >
                  스토어 운영 시작하기! 🚀
                </button>
              )}
          ...
          <div className="scrollable-body">
            {currentStep.type === 'info' ? (
              ...
              <section className="full-panel">
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', padding: '20px', maxWidth: '800px', margin: '0 auto' }}>
                  {currentStep.quizOptions?.map((opt, i) => (
                    <button key={i} onClick={() => handleQuizSelect(opt)}
                      style={{
                        minHeight: '80px', fontSize: '1.2rem', fontWeight: 700, borderRadius: '16px',
                        border: selectedQuizOpt === opt ? (opt === currentStep.answer ? '4px solid #10b981' : '4px solid #ef4444') : '2px solid #e2e8f0',
                        backgroundColor: selectedQuizOpt === opt ? (opt === currentStep.answer ? '#f0fdf4' : '#fef2f2') : 'white',
                ...
                {quizFeedback && <div style={{ marginTop: '2rem', textAlign: 'center', fontSize: '1.2rem', fontWeight: 800, color: quizFeedback.isCorrect ? '#10b981' : '#ef4444' }}>{quizFeedback.message}</div>}
              </section>
            ) : (
              <>
                  ...
                  <div className="practice-grid">
                    ...
                    <div className="simulator-box">
                      ...
                      <div className="simulator-content">
                        ...
                        {isCleared && <div style={{color: '#10b981', marginTop: '1rem', fontWeight: 800}}>✅ {currentStep.id === 7 && shelfState?.[0]?.is_sale === false ? "품절 상태로 설정되었습니다!" : "미션 성공! [다음]을 누르세요."}</div>}
                      </div>
                    </div>
                  </div>
                </section>
      ...
      <AnimatePresence>
        {isFinalStepCleared && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
            style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', backgroundColor: 'rgba(255, 255, 255, 0.98)', zIndex: 1000, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
            <motion.div initial={{ scale: 0.5 }} animate={{ scale: 1 }} transition={{ type: 'spring' }}><Trophy size={100} color="#f59e0b" style={{ marginBottom: '20px' }} /></motion.div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px', maxWidth: '700px', width: '100%', marginBottom: '40px' }}>
              {[
                ...
                <div key={idx} style={{ padding: '20px', borderRadius: '20px', backgroundColor: '#f8fafc', border: '1px solid #e2e8f0' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px', color: '#3b82f6', fontWeight: 800, marginBottom: '8px' }}>{item.icon} {item.title}</div>
                  <div style={{ color: '#475569', fontSize: '0.95rem' }}>{item.desc}</div>
                </div>
              ))}
            </div>
            <button className="btn-base btn-next active" onClick={() => alert("Stage 2는 준비 중입니다!")} style={{ padding: '15px 40px', fontSize: '1.2rem', cursor: 'pointer' }}>Stage 2 도전하기</button>
          </motion.div>
        )}
      </AnimatePresence>
